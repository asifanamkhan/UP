...
@section('head_content')
    স্যানিটেশন ধরন অনুযায়ী পরিবারের তালিকা
@endsection
@section('content')
    <div class="panel panel-default mb-5">
        ...
        <div class="panel-body" >
            <div class="row">
                ...
                <div class="col-sm-12" style="margin-bottom:10px;margin-top:10px;">
                    <div class="form-group has-feedback">
                        <label for="Delivery-type" class="col-sm-3 control-label text-right">স্যানিটেশন ধরন  <span style="color: red">*</span></label>
                        <div class="col-sm-8 has-feedback" id="">
                            <select name="sanitationDhoron" id="sanitationDhoron" class="form-control col-sm-8 text-left">
                                <option value="0">চিহ্নিত করুন</option>
                                @foreach($sanitationDhorons as $sanitationDhoron)
                                    @if($sanitationDhoron->status==1)
                                        <option value="{{$sanitationDhoron->id}}">{{$sanitationDhoron->	sanitation}}</option>
                                    @endif
                                @endforeach
                            </select>
                        </div>
...
@section('script')
    ...
    <script>
        ...
        $('#submit').on('click',function () {
            ...
            var url = "{{route('sdcSanitationDhoronChart')}}";
            var Years = new Array();
                ...
                $.post(url,
                    {
                        _token: "{{ csrf_token() }}",
                        word_no: $('#word_no').val(),
                        sanitationDhoron:$('#sanitationDhoron').val(),
                    },
                    function(response)
                    {
                        Years.push(response[2]);
                        Labels.push(response[3]);
                        Prices.push(response[3]);
                        ...
                        response[1].forEach(function(i,e){
                            Labels.push(i);
                            Prices.push(i);
                        });
                    ...
                    var myChart = new Chart(ctx, {
                        ...
                        data: {
                            labels:Years,
                            datasets: [{
                                label: 'স্যানিটেশন ধরন অনুযায়ী পরিবারের',
                                data: Prices,
                                borderWidth: 1,
            ...
            $('#example'). DataTable( {
                ...
                "ajax":{
                    "url": "{{ route('sdcSanitationDhoronShow') }}",
                    "dataType": "json",
                    "type": "POST",
